 ...
 * This module provides helper functions for accessing and manipulating game state
 * Includes utilities for health calculations, targeting, and room management
 */
 ...
 * Card definition with all properties
 */

/**
 * @typedef {import('./dungeon.js').Dungeon} Dungeon
 * Dungeon map and room definitions
 */

/**
 * @typedef {import('./dungeon.js').MapNode} MapNode
 * A node in the dungeon map representing a room or passage
 */

/**
 * @typedef {import('./rooms.js').Room} Room
 * Room definition with properties and state
 */

/**
 * @typedef {import('./monster.js').MONSTER} MONSTER
 * Monster definition with properties and state
 */
...
/**
 * Validates if a card's target type matches the selected target
 * @param {CardTargets} cardTarget - The type of target the card requires
 * @param {string} targetQuery - The actual target being checked
 * @returns {boolean} True if target is valid for the card
 */
